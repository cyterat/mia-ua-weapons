...
def region_population_end2021(region):
    ...
    return st.markdown(
        f"<span title='In 2021'><small>Population</small></span><br><font size='5.5'>{int(pop):,}</font>",
...
def region_total_end2021(region):
    ...
    return st.markdown(
        f"<span title='Number of records in {current_yr} compared to previous year'><small>Records<br>in {current_yr}</small></span><br><font size='5.5'>{current_total:,}</font><br><font size='2'><span style='color: {delta_color}'>{sign}{yr_diff:,}</span></font>",
        unsafe_allow_html=True,
    )
...
def region_yr_totals_linegarph(region):
    df = region_tl[(region_tl["region"] == str(region))]
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=None, showticklabels=True, gridcolor="#484848", zeroline=False
        ),
        hovermode="x unified",
        hoverlabel=dict(bgcolor="rgba(72,72,72,0.8)", font_color="#F0F0F0"),  # 484848
    )
...
def region_yr_totals_linegrph2012(region):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=theft_tr["theft"],
            name="Theft",
            line=dict(color=biclr[0], width=3),
            marker=dict(size=6),
            mode="lines+markers",
            hovertemplate=htext,
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=loss_tr["loss"],
            name="Loss",
            line=dict(color=biclr[1], width=3),
            marker=dict(size=6),
            mode="lines+markers",
            hovertemplate=htext,
        )
...
def region_total(region):
    ...
    return st.markdown(
        f"<font size='4'>Total records: <span style='color: {clr}'>{total:,}</span></font>",
        unsafe_allow_html=True,
    )
...
def region_theftloss_piechart(region):
    ...
    patches, texts, pcts = ax.pie(
        ...
        startangle=90,
        pctdistance=1.2,
        wedgeprops=dict(width=0.55, edgecolor="#414141", lw=3),
        # Segment labels font
        textprops=dict(weight="bold", size=8),
    )
