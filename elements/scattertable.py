...
def scatter_table():
    ...
    fig = px.scatter(
        ...
        color_continuous_scale=[
            #Colorbar ranges
            [0, favcol[0]], [0.001, favcol[0]], # 0%-0.1% (smallest values & colorbar bottom)
            [0.001, favcol[1]], [0.01, favcol[1]], # 0.1%-1%
            [0.01, favcol[2]], [0.05, favcol[2]], # 1%-5%
            [0.05, favcol[3]], [0.2, favcol[3]], # 5%-20%
            [0.2, favcol[4]], [1, favcol[4]] # 20%-100% (largest values & colorbar top)
        ],
        hover_data={
            'region':False,
            ...
            'freqLog100':False,
            # custom data
            '% of Region Total':(':.3%', cat_ua_df['frequency'] / cat_ua_df['total_region']),
            '% of Country Total':(':.4%', cat_ua_df['frequency'] / cat_ua_df['total_country'])
        }
    )
    ...
    fig.update_layout(
        width=1400,
        height=1000,
        plot_bgcolor= "#333333", #rgba(0,0,0,0)
        ...
        yaxis = dict(
            title=None,
            gridcolor='#484848',#484848 #333333
            tickfont_color="#dedede",
            tickfont_size=14,
            tickfont_family='Verdana, sans-serif',
            ticks="outside"
